...
class LocalOutputWriterTest extends CDAClientTestSpec {
  ...
  describe("Testing OutputWriter functionality") {
    ...
    val testWriter = OutputWriter(OutputWriterConfig(testWriterPath, includeColumnNames = false, saveAsSingleFileCSV = false, saveIntoTimestampDirectory = false))
    val testWriterWithHeader = OutputWriter(OutputWriterConfig(testWriterPath, includeColumnNames = true, saveAsSingleFileCSV = false, saveIntoTimestampDirectory = false))
    val testWriterWithTimestamp = OutputWriter(OutputWriterConfig(testWriterPath, includeColumnNames = true, saveAsSingleFileCSV = true, saveIntoTimestampDirectory = true))
    ...
    describe("OutputWriter.validate") {
      it("should validate folders that can be written to and otherwise throw exceptions") {
        ...
        val writer2 = OutputWriter(OutputWriterConfig(writer2path, includeColumnNames = false, saveAsSingleFileCSV = false, saveIntoTimestampDirectory = false))
        a[IOException] should be thrownBy writer2.validate()

        val writer3path = testManifestPath
        val writer3 = OutputWriter(OutputWriterConfig(writer3path, includeColumnNames = false, saveAsSingleFileCSV = false, saveIntoTimestampDirectory = false))
    ...
    describe("OutputWriter.writeCSV") {
      it("should correctly write a DataFrame to a csv format and save it in a folder with the name of the table and schema fingerprint") {
        ...
        val testDataFrameWrapperForMicroBatch = DataFrameWrapperForMicroBatch(testTableName, testSchemaFingerprint, manifestLastSaveTimestamp, testDF)
      ...
      it("should include column names on the first line of the csv output when this option is specified in the config") {
        val testDF = makeDummyDataFrame(letters, numbers)
        val testDataFrameWrapperForMicroBatch = DataFrameWrapperForMicroBatch(testTableNameWithHeader, testSchemaFingerprint, manifestLastSaveTimestamp, testDF)
    ...
    describe("OutputWriter.makeCSVPath") {
      it("should correctly create the path for a local csv given the table name and fingerprint") {
        val tableDataFrameWrapperForMicroBatch = DataFrameWrapperForMicroBatch(testTableName, testSchemaFingerprint, manifestLastSaveTimestamp, null)
